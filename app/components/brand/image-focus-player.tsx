...
import { getSignCategory } from "~/lib/image-focus";
...
export function ImageFocusPlayer({ className }: { className?: string }) {
  ...
  const { settings, phase, pos, queue, running } = s;
  ...
  const orderLabel =
    settings.order === "shuffle"
      ...
      : settings.reverse
        ? "reverse order"
        : "in order";
  const configSummary = `${total} sign${total === 1 ? "" : "s"} · ${orderLabel}${settings.loop ? " · loop" : ""}${settings.selfGrading ? " · self-grading" : ""}`;
    ...
    return (
      <div className={cn("space-y-3", className)}>
        <div className="relative">
          ...
          />
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-ink/45 backdrop-blur-[1px] text-center">
    ...
    >
      ...
      >
        ...
        <span className="whitespace-nowrap bg-ink/70 px-2 py-0.5 font-mono text-[10px] font-bold tracking-wider tabular-nums text-paper">
          {pos + 1} / {queue.length}
          {settings.selfGrading ? ` · ✓${masteredCount}` : ""}
        </span>
        <Button
          variant="paper"
          size="sm"
      ...
      <AnimatePresence>
        {phase === "complete" && (
          <motion.div
            key="complete"
            className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-3 bg-ink/65 text-center"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            >
              <motion.span
                ...
                className="font-mono text-[12px] uppercase tracking-widest text-paper"
              >
              ...
              <motion.div variants={ANSWER_ITEM} className="flex gap-2">
                <Button variant="rush" size="lg" onClick={() => s.start()}>
                  <HugeiconsIcon
                    icon={RefreshFreeIcons}
                    size={18}
                ...
                <Button variant="paper" size="lg" onClick={() => s.exit()}>
                  Done
                </Button>
              </motion.div>
            </motion.div>
              ...
              {settings.selfGrading ? (
                <motion.div variants={ANSWER_ITEM} className="space-y-3">
                  ...
                  <p className="font-mono text-[10px] uppercase tracking-widest text-ink-3">
                    Missed → comes back sooner. Knew it 3× → mastered.
