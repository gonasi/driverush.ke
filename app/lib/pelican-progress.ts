 ...
 * Missed → box 0 (re-queued soon by the engine); knew → box+1; box ≥
 * {@link MASTERED_BOX} counts as "mastered". Persisted by the Zustand store in
 * `pelican-store.ts`; this file is just the types + pure helpers.
 */
...
/** Apply a grade to a single region's box (pure). */
export function applyGrade(
  ...
  const box = result === "missed" ? 0 : Math.min(MAX_BOX, cur.box + 1);
